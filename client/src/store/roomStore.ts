...
interface OpponentActivity {
  isRunning: boolean; lastStatus: SubmissionStatus | null; executionTime: number | null;
}

interface RoomState {
  room: RoomResponse | null; selectedLanguage: Language;
  code: string; isSubmitting: boolean; opponentActivity: OpponentActivity;
...
export const useRoomStore = create<RoomState>((set) => ({
  room: null, selectedLanguage: "java", code: "", isSubmitting: false,
  ...
  setOpponentActivity: (activity) =>
    set((s) => ({ opponentActivity: { ...s.opponentActivity, ...activity } })),
  clearRoom: () =>
    set({ room: null, code: "", isSubmitting: false,
          opponentActivity: { isRunning: false, lastStatus: null, executionTime: null } }),
}));
