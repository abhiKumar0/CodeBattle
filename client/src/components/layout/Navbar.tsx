...
import { Swords, Trophy, LayoutDashboard, LogOut, Users, BookOpen, User, Eye } from "lucide-react";
  ...
  return (
    <nav className="sticky top-0 z-50 h-12 border-b border-border bg-card/90 backdrop-blur-sm flex items-center px-4 gap-4"
      ...
      <Link href="/dashboard" className="flex items-center gap-2 mr-4 shrink-0 group">
        ...
        <span className="font-mono text-sm font-bold text-green-500 tracking-widest hidden sm:block">
          CODEBATTLE
        </span>
      </Link>
        ...
        {NAV_LINKS.map(({ href, label, icon: Icon }) => {
          ...
          return (
            <Link key={href} href={href}
              className={`flex items-center gap-1.5 px-3 py-1.5 text-xs font-mono font-medium tracking-wider transition-all duration-200 border-b-2 whitespace-nowrap ${
                active
      ...
      <div className="flex items-center gap-2 shrink-0">
        <div className="hidden sm:flex items-center gap-2 font-mono text-xs border border-green-500/20 px-2.5 py-1 bg-green-500/5">
